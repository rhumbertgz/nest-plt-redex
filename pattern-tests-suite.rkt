...
(define (generate-valid-sequences n logic_vars selectors guard context operators)
  ...
  (cond
    [(equal? (length selectors) 1)
     (for/fold ([acc empty])
               ([x (in-range n)])      
       (append acc (list (basic-message-sequence (first selectors) logic_vars guard operators))))       
     ]
    [(sequencing-pattern? (hash-values context)) 
       (for/fold ([acc empty])
               ([x (in-range n)])  
               (let ([msgs (composite-message-sequence selectors logic_vars guard operators)])
                (append acc (list msgs)))
               )
       ]
    [else 
      (let ([seqs (for/fold ([acc empty])
               ([x (in-range (ceiling (/ n 2)))])  ;; the other half will randomly generated by shuffling the sequences generated 
               (let ([msgs (composite-message-sequence selectors logic_vars guard operators)])
                (append acc (list msgs) (list (shuffle-messages msgs)))
               ))])
       ;; if we generated n+1 sequences due to the use of 'ceiling', we discard the oldest sequence
       (cond 
        [ ( > (length seqs) n) (rest seqs)]
        [else seqs])
       )
      ]))

(define (shuffle-messages msgs)
  (let ([shuffled (shuffle msgs)])
    (cond
      [(equal? msgs shuffled) (shuffle-messages msgs)]
      [else shuffled])))

;; s - selector
;; h - logic variables values
;; 0 - operators
(define (basic-message-sequence s lvars g o)
  (log "new-selector-messages"  #:label "call")
  (cond
    [(equal? (hash-has-key? o (first s)) #false) 
     (list (new-elementary-pattern-message s lvars g))]
    [else 
       ...
       (cond
         [(member-of? op (list 'count 'every)) 
          (for/fold ([acc empty])
            ...
            (append acc (list (new-elementary-pattern-message s lvars g)))) ]
         [(equal? op 'window) 
          (first (let* ([n 2] ; (random 1 11)
                   ...
                   (for/fold ([acc (list empty fts)])
                     ...
                     (let* ([lm (first acc)]
                            [ts (second acc)]
                            [m (new-elementary-pattern-message s lvars g ts)]
                            [nlm (append lm (list m))])
                       (list nlm (+ ts tick))
                       ))))]
         [else (error (string-append "Invalid pattern operator " (any->string op)))]))]))

(define (new-elementary-pattern-message s logic_vars guard [ts (current-milliseconds)])
...
(define (composite-message-sequence selectors lvars guard operators)
  (log "composite-message-sequence"  #:label "call")

  (let ([lvars_values (random-value-generator lvars guard)])
    (foldl (lambda (s acc) 
      (let ([msgs (cond
        [(equal? (hash-has-key? operators (first s)) #false) 
          (list (new-composite-pattern-message s lvars_values))]
        [else 
          (let* ([op_exp (hash-ref operators (first s))]
                  [op (first op_exp)])
            (cond
              [(member-of? op (list 'count 'every)) 
                (for/fold ([acc_x empty])
                          ([x (in-range (second op_exp))])      
                  (append acc_x (list (new-composite-pattern-message s lvars_values)))) ]
              [(equal? op 'window) 
                (first (let* ([n (random 1 11)] ;; generate up to 10 random messages in the specified window 
                              [t (first-tstamp-and-tick n (second op_exp) (third op_exp))]
                              [fts (first t)]
                              [tick (second t)])
                        (for/fold ([acc_y (list empty fts)])
                                  ([x (in-range n)])
                          (let* ([lm (first acc_y)]
                                  [ts (second acc_y)]
                                  [m (new-composite-pattern-message s lvars_values ts)]
                                  [nlm (append lm (list m))])
                            (list nlm (+ ts tick))
                            ))))]
              [else (error (string-append "Invalid pattern operator " (any->string op)))]))])])
...
(define (new-composite-pattern-message selector lvar_values [timestamp (current-milliseconds)])
  (log "new-composite-pattern-message" #:label "call")

  (list timestamp (foldl (lambda (x acc)
                      (cond
                        [(and (symbol? x) (hash-has-key? lvar_values x)) (append acc (list (salt-value (hash-ref lvar_values x))))]
                        [else (append acc (list x))])
                      ) empty selector)))
...
(define (first-tstamp n)
  (- (current-milliseconds) (* 1000 n)))
...
;; add a small modification to the logic variable' value without break its existent constrains
(define (salt-value v)
  (cond
    [(number? v) (+ v (exact->inexact (/ (random 1 10) 10) ))]
    [else v]))
