...
(require redex  "nest-utils.rkt" "nest-syntax.rkt" "nest-reductions.rkt")
...
(define (generate-message-sequences p n pollute?)
  ...
  (let* ([selectors (hash-ref p 'selectors)]
    ...
    (cond
      [pollute? (generate-invalid-sequences n logic_vars selectors guard members operators)]
      [else (generate-valid-sequences n logic_vars selectors guard members operators)])
    ))

(define (generate-valid-sequences n logic_vars selectors guard members operators)
  (log "generate-valid-sequences"  #:label "call")
  (log logic_vars #:label "logic vars")
  (log selectors #:label "selectors")
  (log members #:label "members")
